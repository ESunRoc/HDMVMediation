gen_data <- function(n, p, k, l, p0, cov_groups = NULL, resp_groups = NULL, coef.seed = 823543, data.seed = 823543){
  ...
  # Function to compute maximum absolute off‑diagonal correlation for a given sigma2
  max_cor <- function(s2){
  ...
  # Find smallest common sigma2 such that max |cor| ≤ 0.3
  if(max_cor(0) <= 0.7){
    sigma2_Y <- 0
  } else{
    # Upper bound search
    upper <- 100 * max(diag(Sigma_sys))
    while(max_cor(upper) > 0.7){
      upper <- upper * 2
    }
    lower <- 0
    for(iter in 1:50){
